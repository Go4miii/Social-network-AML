...
#
# # 1. 读取数据
# df = pd.read_csv('addr_combined.csv')
#
# # 2. 按时间步分割数据并处理每个时间步
...
#
#     # 构建有向图
#     G = nx.from_pandas_edgelist(sub_df, 'input address', 'output address', create_using=nx.DiGraph())
#
#     # 提取节点特征
#     node_features = []
#     nodes = list(G.nodes)
...
for time_step in time_steps:
    ...
    # 提取原始数据中的非法点
    illicit_nodes_input_address = sub_df[sub_df['class_input_address'] == 1]['input address']
    illicit_nodes_output_address = sub_df[sub_df['class_output_address'] == 1]['output address']
    illicit_nodes = set(illicit_nodes_input_address).union(set(illicit_nodes_output_address))
