...
for i in range(35, 49):
    train_data = known_data[known_data['Timestep'] <= i]
    test_data = known_data[known_data['Timestep'] == i + 1]
    ...
    # X_train = train_data.drop(['address', 'class'], axis=1)
    # y_train = train_data['class']
    print("After dropping:", X_train.shape)

    # if X_train.empty:
    #     raise ValueError("X_train is empty after dropping columns.")
    ...
    X_train = pd.DataFrame(X_train, columns=[col for col in train_data.drop(['address', 'class'], axis=1).columns])
    X_test = pd.DataFrame(X_test, columns=[col for col in test_data.drop(['address', 'class'], axis=1).columns])

    for name, model in models.items():
        # 结合之前的逻辑，定义一个列表来跟踪加入训练集的unknown数据索引
        added_unknown_indexes = []
        for round in range(3):  # 假设迭代5次

            if round > 0:
                # 通过索引移除上一次迭代中加入的unknown数据
                X_train = X_train.drop(added_unknown_indexes, errors='ignore')
                y_train = y_train.drop(added_unknown_indexes, errors='ignore')
            ...
            unknown_data = data[(data['class'].isnull()) & (data['Timestep'] <= i)]
            unknown_data_X = imputer.transform(unknown_data.drop(['address', 'class'], axis=1))
